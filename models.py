...
class MLP(nn.Module):
    def __init__(self, x_dim, y_dim, h_dim=32):
    ...
    def functional_forward(self, x, w_list):
        """
        x : torch.Tensor
            Shape (batch_size, x_dim)
        """
        assert(len(w_list) == len(self.functional_layers))

        for wd, f in zip(w_list, self.functional_layers):
            x = f(x, **wd)
        return x
